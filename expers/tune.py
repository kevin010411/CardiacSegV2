...
def main_worker(args):
    ...
    if args.checkpoint is not None and os.path.exists(args.checkpoint):
        ...
        print(
            "=> loaded checkpoint '{}' (epoch {}) (bestacc {}) (early stop count {})".format(
                args.checkpoint, start_epoch, best_acc, early_stop_count
            )
        )
    else:
        # ssl pretrain
        if (
            args.model_name == "swinunetr"
            and args.ssl_checkpoint
            and os.path.exists(args.ssl_checkpoint)
        ):
            ...
            if "net" in list(weight["state_dict"].keys())[0]:
                ...
                for key in list(weight["state_dict"].keys()):
                    ...
                    if "linear" in new_key:
                        weight["state_dict"][new_key.replace("linear", "fc")] = weight[
                            "state_dict"
                        ].pop(new_key)
            ...
            print(
                f"Using pretrained self-supervied {args.model_name} backbone weights !"
            )
            print("=> loaded pretrain checkpoint '{}'".format(args.ssl_checkpoint))
    ...
    if not args.test_mode:
        ...
        eval_tt_dice_val_df = pd.DataFrame(
            tt_dc_vals, columns=[f"tt_dice{n}" for n in label_names]
        )
        eval_tt_hd95_val_df = pd.DataFrame(
            tt_hd95_vals, columns=[f"tt_hd95{n}" for n in label_names]
        )

        eval_inf_dice_val_df = pd.DataFrame(
            inf_dc_vals, columns=[f"inf_dice{n}" for n in label_names]
        )
        eval_inf_hd95_val_df = pd.DataFrame(
            inf_hd95_vals, columns=[f"inf_hd95{n}" for n in label_names]
        )
        eval_inf_sensitivity_val_df = pd.DataFrame(
            inf_sensitivity_vals, columns=[f"inf_sensitivity{n}" for n in label_names]
        )
        eval_inf_specificity_val_df = pd.DataFrame(
            inf_specificity_vals, columns=[f"inf_specificity{n}" for n in label_names]
        )
        ...
        eval_df = pd.concat(
            [
                ...
                eval_tt_dice_val_df,
                eval_tt_hd95_val_df,
                eval_inf_dice_val_df,
                eval_inf_hd95_val_df,
                eval_inf_sensitivity_val_df,
                eval_inf_specificity_val_df,
                eval_inf_time_df,
            ],
            axis=1,
            join="inner",
...
if __name__ == "__main__":
    ...
    trainable_with_cpu_gpu = tune.with_resources(
        partial(main, args=args), {"cpu": 1, "gpu": 1}
    )
